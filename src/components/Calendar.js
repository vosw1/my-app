...
const Calendar = ({ selectedDate, onDateChange, events }) => {
    ...
    const renderDays = () => {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            const isEventDay = events.some(event => event.getDate() === day && event.getMonth() === currentMonth && event.getFullYear() === currentYear);
            const isHoliday = holidays.some(holiday => holiday.getDate() === day && holiday.getMonth() === currentMonth && holiday.getFullYear() === currentYear);
